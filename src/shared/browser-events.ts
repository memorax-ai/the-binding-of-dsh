...
export class BrowserRemoteEvents {
  ...
  constructor(
    ...
    private readonly call: (endpoint: string, payload: unknown, signal: AbortSignal) => Promise<RpcResult<unknown>>,
    private readonly listeners: Map<string, Set<RemoteEventListener>>,
    private readonly handlers: Map<string, Set<RemoteEventHandler>>,
    private readonly failed: (error: Error) => void,
  ) {
    socket.addEventListener('message', event => {
      try {
        if (typeof event.data !== 'string') throw new Error('Binary Gateway frame')
        const frame = JSON.parse(event.data)
        if (!record(frame) || typeof frame.streamId !== 'string') throw new Error('Malformed Gateway frame')
        const stream = this.streams.get(frame.streamId)
        if (stream === undefined) return // Cancelled stream, possibly already in flight.
        if (frame.type === 'item') {
          if (stream.done || stream.values.length >= 1024) throw new Error('Gateway stream queue overflow or item after end')
          stream.values.push(frame.value)
        } else if (frame.type === 'end') stream.done = true
          ...
          stream.done = true
        } else throw new Error('Malformed Gateway stream frame')
        stream.wake?.()
  ...
  async start(signal: AbortSignal): Promise<void> {
    ...
    try {
      if (this.socket.readyState !== 1) await new Promise<void>((resolve, reject) => {
        ...
        const aborted = (): void => finish(this.lifetime.signal.reason)
  ...
  async *stream(endpoint: string, payload: unknown, signal?: AbortSignal): AsyncGenerator<unknown> {
    if (this.disposed) throw this.lifetime.signal.reason
    if (signal?.aborted) throw signal.reason
    if (this.streams.size >= 256) throw new Error('Gateway stream limit reached')
    const id = globalThis.crypto.randomUUID()
    const stream: Stream = { values: [], done: false }
    let cancelled = false
    const cancel = (): void => {
      this.streams.delete(id)
      if (cancelled || this.disposed || this.socket.readyState !== 1) return
      cancelled = true
      try { this.socket.send(JSON.stringify({ type: 'cancel', streamId: id })) }
      catch (error) { this.fail(error) }
    }
    const abort = (): void => { stream.error = signal?.reason; stream.done = true; stream.wake?.(); cancel() }
    ...
    try {
      ...
      while (true) {
        ...
        if (stream.values.length) { yield stream.values.shift(); continue }
  ...
  dispose(reason: unknown = new Error('Gateway generation closed')): void {
    if (this.disposed) return
    this.disposed = true
    this.lifetime.abort(reason)
    for (const pending of this.pending.values()) pending.abort(reason)
    this.pending.clear()
  ...
  private async pump(events: AsyncIterator<unknown>): Promise<void> {
    while (!this.disposed) {
      const next = await events.next()
      if (next.done) throw new Error('Gateway event stream ended')
      const frame = next.value
      if (!record(frame)) throw new Error('Malformed Remote event')
      if (frame.type === 'emit' && typeof frame.event === 'string' && Array.isArray(frame.args)) {
        for (const listener of this.listeners.get(frame.event) ?? []) {
          void Promise.resolve().then(() => listener(...frame.args as unknown[])).catch(error => {
        ...
        if (this.pending.has(frame.eventId) || this.pending.size >= 1024) throw new Error('Duplicate or excessive Remote requests')
        void this.dispatch(frame.event, frame.eventId, frame.agentId, frame.request).catch(error => this.fail(error))
      } else throw new Error('Malformed Remote event')
    }
  }

  private async dispatch(event: string, eventId: string, agentId: string, request: Record<string, unknown>): Promise<void> {
    const abort = new AbortController()
    this.pending.set(eventId, abort)
    let outcome: Record<string, unknown> = { kind: 'next' }
    try {
      for (const handler of this.handlers.get(event) ?? []) {
        const value = await handler({ eventId, agentId, request, signal: abort.signal })
        if (abort.signal.aborted) return
        if (value !== undefined) { outcome = { kind: 'result', value }; break }
      }
    } catch (error) {
    ...
    try {
      ...
      if (!result.ok) throw new Error('Gateway rejected Remote event result')
    } catch (error) {
      if (!abort.signal.aborted) throw error
    } finally {
      if (this.pending.get(eventId) === abort) this.pending.delete(eventId)
    }
  }
}
